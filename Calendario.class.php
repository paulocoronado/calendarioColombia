...
* @link		N/D
* @description  Calendario Colombia
* @usage
*******************************************************************************/
class Calendario
{
	...
	function buscar_lunesFestivo($dia,$mes,$anno)
	{
		$buscar=$this->cadena_busqueda_calendario(3,$configuracion,$dia,$mes);
		...
		if($campos>0)
		{
			for($i=0;$i<$campos;$i++)
			{
				$esta_fecha=strtotime($registro_festivo[$i][2]."/".$registro_festivo[$i][1]."/".$anno);
				//Se corre al lunes siguiente.
				//Determinar el dia de la semana...

				$este_dia_semana = date("w",$esta_fecha);

				//echo $este_dia_semana."<br>";
	...
	function  buscar_festivo($dia,$mes,$anno,$configuracion)
	{
		...
		$buscar=$this->cadena_busqueda_calendario(1,$configuracion,$dia,$mes);
		//echo $buscar."<hr>";

		//echo $pascua["mes"]."/".$pascua["dia"]."<br>";
		...
		{
			if(date("w",$esta_fecha)==1)
			{
				return $this->buscar_lunesFestivo($dia,$mes,$anno,$configuracion, $acceso_db);
			}
			else
			{
				...
				else
				{
					...
					else
					{
						...
						{
							$es_festivo["dia"]=$dia;
							$es_festivo["descripcion"]="Domingo";
							$es_festivo["mes"]=$mes;
							$es_festivo["anno"]=$anno;
							return $es_festivo;
						}
